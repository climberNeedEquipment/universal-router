...
abstract contract Payments is RouterImmutables {
    ...
    function wrapETH(address recipient, uint256 amount) internal returns (uint256 amountOut) {
    ...
    function unwrapWETH9(address recipient, uint256 amount) internal returns (uint256 amountOut) {
        if (WETH9.balanceOf(address(this)) < amount) {
            revert InsufficientETH();
        }
